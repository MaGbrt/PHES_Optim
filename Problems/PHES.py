...
class PHES(Single_Objective):
    ...
    #-------------__init__-------------#    
    def __init__(self, n_dvar, day_i, fill_ratio):
        self._day = day_i
    ...
    def perform_real_evaluation(self, candidates):
        ...
        """
        start = time()
        assert self.is_feasible(candidates), print(candidates)
        DAM_d = np.zeros(24)
        ...
        if candidates.ndim==1:
            ...
            instruction = 'evaluate_d(' + str(DAM_d) + ', ' + str(reserves_d) + ', ' + str(self._day) + ', ' + str(self._fill_ratio) + ')'
